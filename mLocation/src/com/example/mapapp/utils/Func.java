...
import com.example.mlocation.impl.BaiDuMapHandler;
import com.example.mlocation.impl.LauncherListener;
import com.example.mlocation.impl.MapHandler;
import com.example.mlocation.impl.UserAndMapHandler;
...
public class Func {
	...
	/**
	 * UserAndMapInfo json转对象
	 * 
	 * @param implString
	 * @return
	 */
	public static UserAndMapInfo userAndMapInfo(String implString) {
		UserAndMapInfo implInfos = null;
		try {
			Gson gson = new Gson();
			implInfos = gson.fromJson(implString, UserAndMapInfo.class);
		} catch (JsonIOException e) {
			// TODO: handle exception
			e.printStackTrace();
		} catch (JsonSyntaxException e) {
	 ...
	 * @return
	 */
	public static MapInfo mapInfo(String implString) {
		MapInfo implInfos = null;
		try {
			Gson gson = new Gson();
			implInfos = gson.fromJson(implString, MapInfo.class);
		} catch (JsonIOException e) {
			// TODO: handle exception
			e.printStackTrace();
		} catch (JsonSyntaxException e) {
	...
	/**
	 * BaiDuMapByIp json转对象
	 * 
	 * @param implString
	 * @return
	 */
	public static BaiDuMapByIp baiDuMapByIp(String implString) {
		BaiDuMapByIp implInfos = null;
		try {
			Gson gson = new Gson();
			implInfos = gson.fromJson(implString, BaiDuMapByIp.class);
		} catch (JsonIOException e) {
			// TODO: handle exception
			e.printStackTrace();
		} catch (JsonSyntaxException e) {
	 ...
	 * @param postParam
	 */
	public static void getUserAndMapInfos(Context context, String url,
			LauncherListener launcherListener, String postParam) {
		Func.FUNC_TASK.execute(new UserAndMapHandler(context, url,
				launcherListener, postParam));
	}
	 ...
	 * @param postParam
	 */
	public static void getMapInfos(Context context, String url,
			LauncherListener launcherListener, String postParam) {
		Func.FUNC_TASK.execute(new MapHandler(context, url, launcherListener,
				postParam));
	}

	/**
	 * 开启线程请求数据 BaiDuMapInfo
	 * 
	 * @param context
	 * @param url
	 * @param launcherListener
	 */
	public static void getBaiDuMapByIpInfos(Context context, String url,
			LauncherListener launcherListener) {
		Func.FUNC_TASK.execute(new BaiDuMapHandler(context, url,
				launcherListener));
	}
}
