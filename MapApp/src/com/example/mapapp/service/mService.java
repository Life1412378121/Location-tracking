...
public class mService extends Service {
	...
	public class MyLocationListenner implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			Log.i("test", "addr=" + location.getAddrStr());
			...
			if (location != null && !TextUtils.isEmpty(location.getAddrStr())
					&& !addr.equals(addr2)) {
				final String postStr = Func.postParam(context, "1", time,
						latitude, longitude, addr, Func.getMac());
				...
				Func.FUNC_TASK.execute(new Runnable() {
					...
					public void run() {
						// TODO Auto-generated method stub
						Func.postImplRequest(context, url, postStr);
					}
				});
